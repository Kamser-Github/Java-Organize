...
여기서 조건식이 참이면 실행문 1이 실행
       조건식이 거짓이면 실행문 2가 실행
따라서 조건 기준은 조건식의 참/거짓으로 따진다.
...
여기서 조건기준은 여러개가 올수있다.
우선순위가 
조건식 1을 만족하면 실행문 1을 실행하고 끝난다.
그다음 조건식 2를 만족하면 실행문 2를 실행하고 끝난다.
조건식 1,2,3,4를 모두 만족하지 않는건 
다 else문 실행문 5가 된다.

조건의 기준이 여러개이며 조건이 여러개 일경우
예외먼저 처리시켜서 기준이 안되는건 빠르게 제외시키는 좋다.
